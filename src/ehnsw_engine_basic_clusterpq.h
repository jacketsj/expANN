...
struct ehnsw_engine_basic_clusterpq_config {
	size_t M;
	size_t M0;
	...
	ehnsw_engine_basic_clusterpq_config(
...
template <typename T>
struct ehnsw_engine_basic_clusterpq
		: public ann_engine<T, ehnsw_engine_basic_clusterpq<T>> {
...
#endif
	ehnsw_engine_basic_clusterpq(ehnsw_engine_basic_clusterpq_config conf)
			: rd(), gen(0), distribution(0, 1), M(conf.M), M0(conf.M0),
				...
				num_centroids(conf.num_centroids), subvector_size(conf.subvector_size),
				max_layer(0) {}
	...
	std::vector<product_quantizer_2::codes_t> vcodes; // vector -> quantized value
	...
	const std::string _name() { return "(noE)HNSW Engine Basic Cluster-PQ"; }
...
template <typename T>
std::vector<std::pair<T, size_t>> ehnsw_engine_basic_clusterpq<T>::prune_edges(
...
template <typename T>
void ehnsw_engine_basic_clusterpq<T>::_store_vector(const vec<T>& v) {
...
template <typename T> void ehnsw_engine_basic_clusterpq<T>::_build() {
	...
	// assign each vector to a cluster centre
	cluster_membership.resize(all_entries.size());
	std::vector<std::vector<size_t>> centroids_candidates(cluster_centres.size());
	for (size_t v = 0; v < all_entries.size(); ++v) {
		// get the top cluster for v, and use that
		cluster_membership[v] = cluster_engine.query_k(all_entries[v], 1)[0];
		centroids_candidates[cluster_membership[v]].emplace_back(v);
	}
	// populate the quantizers
	for (size_t c = 0; c < centroids_candidates.size(); ++c) {
		std::shuffle(centroids_candidates[c].begin(), centroids_candidates[c].end(),
								 gen);
...
template <bool use_bottomlayer>
std::vector<std::pair<T, size_t>>
ehnsw_engine_basic_clusterpq<T>::query_k_at_layer(
...
template <typename T>
std::vector<size_t> ehnsw_engine_basic_clusterpq<T>::_query_k(const vec<T>& q,
																															size_t k) {
	size_t entry_point = starting_vertex;
#ifdef RECORD_STATS
	++num_distcomps;
...
template <typename T>
std::vector<std::pair<T, size_t>>
ehnsw_engine_basic_clusterpq<T>::query_k_combined(const vec<T>& q, size_t k) {
	size_t entry_point = starting_vertex;
#ifdef RECORD_STATS
	++num_distcomps;
#endif
