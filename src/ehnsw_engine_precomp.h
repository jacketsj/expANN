...
struct ehnsw_engine_precomp_config {
	size_t edge_count_mult;
	size_t num_for_1nn;
	size_t edge_count_search_factor;
	ehnsw_engine_precomp_config(size_t _edge_count_mult, size_t _num_for_1nn,
...
template <typename T, size_t dim, size_t edge_count_mult,
					size_t edge_count_search_factor, size_t knn_count>
struct ehnsw_engine_precomp {
	std::random_device rd;
	std::mt19937 gen;
	std::uniform_real_distribution<> distribution;
	std::uniform_int_distribution<> int_distribution;
	size_t starting_vertex;
	static constexpr size_t num_cuts = edge_count_mult - 1;
	using svec = Eigen::Matrix<float, dim, 1>;
	ehnsw_engine_precomp(ehnsw_engine_precomp_config conf)
			: rd(), gen(rd()), distribution(0, 1), int_distribution(0, 1) {}
	std::vector<svec> all_entries;
	struct layer_data {
		// TODO adj can be std::array
	...
	// TODO e_labels can be std::array (thought doesn't affect queries)
	std::vector<std::vector<bool>>
			e_labels; // data_index -> cut labels (*num_cuts=edge_count_mult-1)
	void _store_vector(const svec& v);
	void _build();
	const std::vector<std::pair<T, size_t>>
	_query_k_internal(const svec& v, size_t k,
										const std::vector<size_t>& starting_points, size_t layer);
	size_t _query_1_internal(const svec& v, size_t starting_point, size_t layer);
	...
	_query_k_internal_wrapper(const svec& v, size_t k,
														size_t full_search_top_layer);
	std::vector<size_t> _query_k(const svec& v, size_t k);
	const std::string _name() { return "EHNSW Engine Precomputed"; }
...
template <typename T>
void ehnsw_engine_precomp<T>::_store_vector(const svec& v) {
...
template <typename T>
bool ehnsw_engine_precomp<T>::is_valid_edge(size_t i, size_t j, size_t bin) {
...
template <typename T>
void ehnsw_engine_precomp<T>::add_edge_directional(size_t i, size_t j, T d,
																									 size_t layer) {
...
template <typename T>
void ehnsw_engine_precomp<T>::add_edge(size_t i, size_t j, T d, size_t layer) {
	add_edge_directional(i, j, d, layer);
	add_edge_directional(j, i, d, layer);
}

template <typename T> void ehnsw_engine_precomp<T>::_build() {
...
template <typename T>
const std::vector<std::pair<T, size_t>>
ehnsw_engine_precomp<T>::_query_k_internal(
		const svec& v, size_t k, const std::vector<size_t>& starting_points,
		size_t layer) {
	auto& adj = layers[layer].adj;
...
template <typename T>
const std::vector<std::vector<std::pair<T, size_t>>>
ehnsw_engine_precomp<T>::_query_k_internal_wrapper(
		const svec& v, size_t k, size_t full_search_top_layer) {
...
template <typename T>
size_t ehnsw_engine_precomp<T>::_query_1_internal(const svec& v,
...
template <typename T>
std::vector<size_t> ehnsw_engine_precomp<T>::_query_k(const svec& v, size_t k) {
	auto current = starting_vertex;
	for (int layer = layers.size() - 1; layer > 0; --layer) {
