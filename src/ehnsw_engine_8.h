...
struct ehnsw_engine_8_config {
	size_t edge_count_mult;
	size_t num_for_1nn;
	size_t edge_count_search_factor;
	ehnsw_engine_8_config(size_t _edge_count_mult, size_t _num_for_1nn,
...
template <typename T>
struct ehnsw_engine_8 : public ann_engine<T, ehnsw_engine_8<T>> {
	...
	size_t num_for_1nn;
	size_t edge_count_search_factor;
	ehnsw_engine_8(ehnsw_engine_8_config conf)
...
template <typename T> void ehnsw_engine_8<T>::_store_vector(const vec<T>& v) {
...
template <typename T>
void ehnsw_engine_8<T>::add_edge_directional(size_t i, size_t j, T d,
																						 size_t layer) {
...
template <typename T>
void ehnsw_engine_8<T>::add_edge(size_t i, size_t j, T d, size_t layer) {
	add_edge_directional(i, j, d, layer);
	add_edge_directional(j, i, d, layer);
}

template <typename T> void ehnsw_engine_8<T>::_build() {
	...
	auto improve_vertex_edges = [&](size_t v) {
		...
		for (size_t layer = 0; layer < std::min(kNNs.size(), vertex_heights[v] + 1);
			...
			std::set<std::pair<T, size_t>> to_add;
			for (auto [d, u_in_layer] : kNNs[layer]) {
				to_add.emplace(d, u_in_layer);
				for (size_t u_neighbour_in_layer : layers[layer].adj[u_in_layer]) {
					// TODO ensure consistent ordering in dist2 computations
					to_add.emplace(dist2(layers[layer].vals[u_neighbour_in_layer],
															 layers[layer].vals[v_in_layer]),
												 u_neighbour_in_layer);
				}
			}
			// for (auto [d, u_in_layer] : kNNs[layer]) {
			for (auto [d, u_in_layer] : to_add) {
...
template <typename T>
const std::vector<std::pair<T, size_t>>
ehnsw_engine_8<T>::_query_k_internal(const vec<T>& v, size_t k,
...
template <typename T>
size_t ehnsw_engine_8<T>::_query_1_internal(const vec<T>& v,
...
template <typename T>
std::vector<size_t> ehnsw_engine_8<T>::_query_k(const vec<T>& v, size_t k) {
	auto current = starting_vertex;
	for (int layer = layers.size() - 1; layer > 0; --layer) {
...
template <typename T> ehnsw_engine_8<T>::~ehnsw_engine_8() {
