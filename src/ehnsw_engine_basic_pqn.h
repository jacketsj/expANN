...
struct ehnsw_engine_basic_pqn_config {
	size_t M;
	size_t M0;
...
template <typename T>
struct ehnsw_engine_basic_pqn
		: public ann_engine<T, ehnsw_engine_basic_pqn<T>> {
...
#endif
	ehnsw_engine_basic_pqn(ehnsw_engine_basic_pqn_config conf)
	...
	const std::string _name() { return "EHNSW Engine Basic PQ-Neighbours"; }
...
template <typename T>
std::vector<std::pair<T, size_t>>
ehnsw_engine_basic_pqn<T>::prune_edges(size_t layer, size_t from,
...
template <typename T>
template <bool use_bottomlayer>
std::vector<std::pair<T, size_t>> ehnsw_engine_basic_pqn<T>::query_k_at_layer(
...
template <typename T>
std::vector<size_t> ehnsw_engine_basic_pqn<T>::_query_k(const vec<T>& q,
																												size_t k) {
	size_t entry_point = starting_vertex;
#ifdef RECORD_STATS
	++num_distcomps;
...
template <typename T>
std::vector<std::pair<T, size_t>>
ehnsw_engine_basic_pqn<T>::query_k_combined(const vec<T>& q, size_t k) {
	size_t entry_point = starting_vertex;
#ifdef RECORD_STATS
	++num_distcomps;
#endif
