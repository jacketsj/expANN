...
struct ehnsw_engine_basic_projn_config {
	size_t M;
	size_t M0;
	size_t ef_search_mult;
	size_t ef_construction;
	size_t num_from_proj;
	ehnsw_engine_basic_projn_config(size_t _M, size_t _M0, size_t _ef_search_mult,
																	size_t _ef_construction,
																	size_t _num_from_proj)
			: M(_M), M0(_M0), ef_search_mult(_ef_search_mult),
				ef_construction(_ef_construction), num_from_proj(_num_from_proj) {}
};

template <typename T>
struct ehnsw_engine_basic_projn
		: public ann_engine<T, ehnsw_engine_basic_projn<T>> {
	...
	std::vector<projection_filter<T>> projection_filters; // TODO populate and use
	...
	const std::string _name() {
		return "EHNSW Engine Basic Neighbour-Projections";
	}
...
template <typename T>
std::vector<std::pair<T, size_t>>
ehnsw_engine_basic_projn<T>::prune_edges(size_t layer, size_t from,
...
template <typename T>
void ehnsw_engine_basic_projn<T>::_store_vector(const vec<T>& v) {
	...
	projection_filters.emplace_back();
	for (size_t layer = 0; layer <= new_max_layer; ++layer) {
...
template <typename T> void ehnsw_engine_basic_projn<T>::_build() {
...
template <typename T>
template <bool use_bottomlayer>
std::vector<std::pair<T, size_t>> ehnsw_engine_basic_projn<T>::query_k_at_layer(
	...
	while (!candidates.empty()) {
		...
		auto get_neighbour_list = [&]() constexpr->neighbour_list_t {
			if constexpr (use_bottomlayer) {
				std::vector<size_t> ret;
				for (auto& [_, i] :
						 projection_filters[cur.second].get_top_k_vectors(q, num_from_proj))
					ret.emplace_back(i);
				return ret;
			} else {
...
template <typename T>
std::vector<size_t> ehnsw_engine_basic_projn<T>::_query_k(const vec<T>& q,
																													size_t k) {
	size_t entry_point = starting_vertex;
#ifdef RECORD_STATS
	++num_distcomps;
...
template <typename T>
std::vector<std::pair<T, size_t>>
ehnsw_engine_basic_projn<T>::query_k_combined(const vec<T>& q, size_t k) {
	size_t entry_point = starting_vertex;
#ifdef RECORD_STATS
	++num_distcomps;
#endif
