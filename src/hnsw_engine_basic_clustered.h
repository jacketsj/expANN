...
struct hnsw_engine_basic_clustered_config {
	size_t M;
	size_t M0;
	size_t ef_search_mult;
	size_t ef_construction;
	size_t cluster_size;
	size_t cluster_neighbours;
	hnsw_engine_basic_clustered_config(size_t _M, size_t _M0,
																		 size_t _ef_search_mult,
																		 size_t _ef_construction,
																		 size_t _cluster_size,
																		 size_t _cluster_neighbours)
			: M(_M), M0(_M0), ef_search_mult(_ef_search_mult),
				ef_construction(_ef_construction), cluster_size(_cluster_size),
				cluster_neighbours(_cluster_neighbours) {}
};

template <typename T>
struct hnsw_engine_basic_clustered
		: public ann_engine<T, hnsw_engine_basic_clustered<T>> {
...
#endif
	bool done_build;
	hnsw_engine_basic_clustered(hnsw_engine_basic_clustered_config conf)
			: rd(), gen(0), distribution(0, 1), M(conf.M), M0(conf.M0),
				ef_search_mult(conf.ef_search_mult),
				ef_construction(conf.ef_construction), cluster_size(conf.cluster_size),
				cluster_neighbours(conf.cluster_neighbours), max_layer(0),
#ifdef RECORD_STATS
				num_distcomps(0),
#endif
				done_build(false) {
	...
	std::vector<std::vector<size_t>> clusters; // cluster index -> vector indices
	std::vector<size_t> to_cluster_index;			 // vector index -> cluster index
	...
	const std::string _name() { return "HNSW Engine Basic Clustered"; }
	const param_list_t _param_list() {
		...
		add_param(pl, cluster_neighbours);
		return pl;
	}
};

template <typename T>
std::vector<std::pair<T, size_t>> hnsw_engine_basic_clustered<T>::prune_edges(
		size_t layer, std::vector<std::pair<T, size_t>> to) {
...
template <typename T>
void hnsw_engine_basic_clustered<T>::_store_vector(const vec<T>& v) {
...
template <typename T> void hnsw_engine_basic_clustered<T>::_build() {
	assert(all_entries.size() > 0);

	// pick some subset of elements to be cluster centres
	std::vector<size_t> cluster_centres;
	for (size_t i = 0; i < all_entries.size(); ++i)
		cluster_centres.emplace_back(i);
	size_t num_clusters = std::max(all_entries.size() / cluster_size, size_t(1));
	std::shuffle(cluster_centres.begin(), cluster_centres.end(), gen);
	cluster_centres.resize(std::min(cluster_centres.size(), num_clusters));

	// assign each element to a cluster
	// do this by indexing the clusters, similar to rcg engine
	...
	for (size_t i = 0; i < cluster_centres.size(); ++i) {
		cluster_engine.store_vector(all_entries[cluster_centres[i]]);
	}
	cluster_engine.build();
	for (size_t i = 0; i < all_entries.size(); ++i) {
		size_t assigned_cluster_index =
				cluster_engine.query_k(all_entries[i], 1)[0];
		to_cluster_index.emplace_back(assigned_cluster_index);
	}
	// TODO: alternative overlap methods
	// - add each element to clusters of k nearest cluster centres
	// "cluster_overlap", to_cluster_index adds a dimension

	// add elements to each cluster, including neighbours of primary elements
	clusters.resize(cluster_centres.size());
	for (size_t i = 0; i < all_entries.size(); ++i) {
		clusters[to_cluster_index[i]].emplace_back(i);
		for (size_t j : _query_k(all_entries[i], cluster_neighbours)) {
			clusters[to_cluster_index[i]].emplace_back(j);
		}
	}
	for (size_t i = 0; i < clusters.size(); ++i) {
		std::set<size_t> cluster_set(clusters[i].begin(), clusters[i].end());
		clusters[i].assign(cluster_set.begin(), cluster_set.end());
	}
...
template <bool use_bottomlayer>
std::vector<std::pair<T, size_t>>
hnsw_engine_basic_clustered<T>::query_k_at_layer(
...
template <typename T>
std::vector<size_t> hnsw_engine_basic_clustered<T>::_query_k(const vec<T>& q,
																														 size_t k) {
	size_t entry_point = starting_vertex;
#ifdef RECORD_STATS
	++num_distcomps;
	...
	// TODO use cluster here to get starting points
	std::vector<size_t> entry_points = {entry_point};
	if (done_build) {
		entry_points = clusters[to_cluster_index[entry_point]];
		std::vector<std::pair<T, size_t>> ret_combined;
		for (auto& ep : entry_points) {
			ret_combined.emplace_back(dist2(all_entries[ep], q), ep);
#ifdef RECORD_STATS
			++num_distcomps;
#endif
		}
		sort(ret_combined.begin(), ret_combined.end());
	...
	auto ret_combined =
			query_k_at_layer<true>(q, 0, entry_points, k * ef_search_mult);
	if (ret_combined.size() > k)
		ret_combined.resize(k);
	std::vector<size_t> ret;
	for (size_t i = 0; i < ret_combined.size() && i < k; ++i) {
