...
bench_data_manager perform_benchmarks(test_dataset_t ds, size_t num_threads) {
	...
	if (true) {
		...
		for (size_t k = 40; k <= 40; k += 10) { // will hopfully get good
			...
			// for (size_t num_for_1nn = 1; num_for_1nn <= 4; num_for_1nn *= 2) {
			for (size_t num_for_1nn = 1; num_for_1nn <= 2; num_for_1nn *= 2) {
				if (false) {
				...
				if (true) {
					for (size_t edge_count_search_factor : {3}) {
						for (size_t num_clusters : {8}) {
							for (size_t cluster_size : {k / 2}) {
	...
	return perform_and_store_benchmark_results(
			...
			ehnsw_engine_basic_pqn_jobs, ehnsw_engine_basic_projn_jobs,
