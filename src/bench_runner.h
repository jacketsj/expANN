...
#include "ehnsw_engine_basic_fast_disk_threaded.h"
...
bench_data_manager perform_benchmarks(test_dataset_t ds, size_t num_threads) {
	...
	std::vector<job<ehnsw_engine_basic_fast_disk_threaded<float>,
									ehnsw_engine_basic_fast_disk_threaded_config>>
			ehnsw_engine_basic_fast_disk_threaded_jobs;
	...
	std::vector<job<ehnsw_engine_basic_clusterpq<float>,
									ehnsw_engine_basic_clusterpq_config>>
			ehnsw_engine_basic_clusterpq_jobs;
	std::vector<
			job<ehnsw_engine_basic_projn<float>, ehnsw_engine_basic_projn_config>>
			ehnsw_engine_basic_projn_jobs;
			...
			for (size_t num_for_1nn = 3; num_for_1nn <= 3; num_for_1nn++) {
				...
				if (false) {
					for (size_t edge_count_search_factor : {3}) {
						...
						for (size_t num_clusters : {1}) {
							for (size_t num_centroids : {500000}) {
								for (size_t subvector_size : {128}) {
									ehnsw_engine_basic_clusterpq_jobs.emplace_back(
											ehnsw_engine_basic_clusterpq_config(
													k, 2 * k, num_for_1nn, k * edge_count_search_factor,
													num_clusters, num_centroids, subvector_size));
								}
							}
						}
					}
				...
				if (false) {
					for (size_t edge_count_search_factor : {3}) {
						ehnsw_engine_basic_fast_disk_threaded_jobs.emplace_back(
								ehnsw_engine_basic_fast_disk_threaded_config(
										k, 2 * k, num_for_1nn, k * edge_count_search_factor, 8,
										100));
					}
				}
				if (false) {
								...
								for (size_t centroid_count : {decltype(k)(16)}) {
									ehnsw_engine_basic_pqn_jobs.emplace_back(
											ehnsw_engine_basic_pqn_config(
													k, 2 * k, num_for_1nn, k * edge_count_search_factor,
													num_from_pq, subvector_size, centroid_count));
								}
							}
						}
					}
