...
struct ehnsw_engine_basic_fast_clusterchunks_pqprune_config {
	size_t M;
	size_t M0;
	...
	ehnsw_engine_basic_fast_clusterchunks_pqprune_config(
...
template <typename T>
struct ehnsw_engine_basic_fast_clusterchunks_pqprune
		: public ann_engine<T, ehnsw_engine_basic_fast_clusterchunks_pqprune<T>> {
...
#endif
	ehnsw_engine_basic_fast_clusterchunks_pqprune(
			ehnsw_engine_basic_fast_clusterchunks_pqprune_config conf)
			: rd(), gen(0), distribution(0, 1), M(conf.M), M0(conf.M0),
				...
				max_layer(0) {}
	using config = ehnsw_engine_basic_fast_clusterchunks_pqprune_config;
	...
	const std::string _name() {
		return use_cuts ? "EHNSW Engine Basic Fast Cluster-Chunks PQ-Prune"
										: "HNSW Engine Basic Fast Cluster-Chunks PQ-Prune";
	}
...
template <typename T>
std::vector<std::pair<T, size_t>>
ehnsw_engine_basic_fast_clusterchunks_pqprune<T>::prune_edges(
...
template <typename T>
void ehnsw_engine_basic_fast_clusterchunks_pqprune<T>::_store_vector(
		const vec<T>& v) {
...
template <typename T>
void ehnsw_engine_basic_fast_clusterchunks_pqprune<T>::_build() {
...
template <bool use_bottomlayer, bool use_clusters>
std::vector<std::pair<T, size_t>>
ehnsw_engine_basic_fast_clusterchunks_pqprune<T>::query_k_at_layer(
...
template <typename T>
std::vector<std::pair<T, size_t>> ehnsw_engine_basic_fast_clusterchunks_pqprune<
		T>::query_k_at_bottom_via_clusters(const vec<T>& q, size_t layer,
...
template <typename T>
std::vector<size_t>
ehnsw_engine_basic_fast_clusterchunks_pqprune<T>::_query_k(const vec<T>& q,
																													 size_t k) {
	size_t entry_point = starting_vertex;
#ifdef RECORD_STATS
	++num_distcomps;
