...
struct static_rcg_engine_simple_config {
	size_t M;
	size_t M0;
	size_t MM;
	size_t ef_search_mult;
	size_t ef_construction;
	static_rcg_engine_simple_config(size_t _M, size_t _M0, size_t _MM,
																	size_t _ef_search_mult,
																	size_t _ef_construction)
			: M(_M), M0(_M0), MM(_MM), ef_search_mult(_ef_search_mult),
				ef_construction(_ef_construction) {}
};

template <typename T>
struct static_rcg_engine_simple
		: public ann_engine<T, static_rcg_engine_simple<T>> {
...
#endif
	static_rcg_engine_simple(static_rcg_engine_simple_config conf)
...
template <typename T>
std::vector<std::pair<T, size_t>>
static_rcg_engine_simple<T>::prune_edges(size_t layer,
...
template <typename T>
void static_rcg_engine_simple<T>::_store_vector(const vec<T>& v) {
...
template <typename T> void static_rcg_engine_simple<T>::_build() {
...
template <typename T>
template <bool use_bottomlayer>
std::vector<std::pair<T, size_t>> static_rcg_engine_simple<T>::query_k_at_layer(
...
template <typename T>
std::vector<std::pair<T, size_t>>
static_rcg_engine_simple<T>::query_k_recursively(
		const vec<T>& q, const std::vector<size_t>& entry_points, size_t k) {
	using measured_data = std::pair<T, size_t>;
...
template <typename T>
std::vector<size_t> static_rcg_engine_simple<T>::_query_k(const vec<T>& q,
																													size_t k) {
	size_t entry_point = starting_vertex;
#ifdef RECORD_STATS
	++num_distcomps;
	...
	for (int layer = max_layer - 1; layer >= 0; --layer) {
		...
		while (changed) {
			changed = false;
			for (auto& neighbour : hadj_flat[entry_point][layer]) {
				// TODO better prefetching here maybe
				_mm_prefetch(&all_entries[neighbour], _MM_HINT_T0);
#ifdef RECORD_STATS
				++num_distcomps;
#endif
	...
	auto ret_combined = query_k_recursively(q, {entry_point}, k * ef_search_mult);
