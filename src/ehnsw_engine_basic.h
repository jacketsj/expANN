...
struct ehnsw_engine_basic_config {
	size_t M;
	size_t M0;
	size_t ef_search_mult;
	size_t ef_construction;
	ehnsw_engine_basic_config(size_t _M, size_t _M0, size_t _ef_search_mult,
...
#endif
	ehnsw_engine_basic(ehnsw_engine_basic_config conf)
...
template <typename T>
std::vector<std::pair<T, size_t>>
ehnsw_engine_basic<T>::prune_edges(size_t layer, size_t from,
...
template <bool use_bottomlayer>
std::vector<std::pair<T, size_t>>
ehnsw_engine_basic<T>::query_k_at_layer(const vec<T>& q, size_t layer,
...
template <typename T>
std::vector<size_t> ehnsw_engine_basic<T>::_query_k(const vec<T>& q, size_t k) {
	size_t entry_point = starting_vertex;
#ifdef RECORD_STATS
	++num_distcomps;
#endif
...
template <typename T>
std::vector<std::pair<T, size_t>>
ehnsw_engine_basic<T>::query_k_combined(const vec<T>& q, size_t k) {
	size_t entry_point = starting_vertex;
#ifdef RECORD_STATS
	++num_distcomps;
#endif
