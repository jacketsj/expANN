...
template <typename T>
std::vector<size_t>
ehnsw_engine_2<T>::_query_k_at_layer_internal(const vec<T>& v, topk_t<T>& tk,
																							size_t layer) {
...
template <typename T>
std::vector<size_t> ehnsw_engine_2<T>::_query_k_internal(const vec<T>& v,
																												 topk_t<T>& tk) {
	tk.consider(dist2(v, all_entries[starting_vertex]), starting_vertex);
	for (int layer = hadj.size() - 1; layer >= 0; --layer) {
		// get kNN at current layer
		std::vector<size_t> kNN = _query_k_at_layer_internal(v, tk, layer);
	}
	return tk.to_vector();
}
...
std::vector<size_t> ehnsw_engine_2<T>::_query_k(const vec<T>& v, size_t k) {
	topk_t<T> tk(k * num_for_1nn);
	auto top_k_mult = _query_k_internal(v, tk);
	top_k_mult.resize(k);
	return top_k_mult;
}
