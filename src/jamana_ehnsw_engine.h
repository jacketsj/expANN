...
struct jamana_ehnsw_engine_config {
	size_t max_depth;
	...
	float elabel_prob;
	float prune_coeff;
	jamana_ehnsw_engine_config(size_t _max_depth, size_t _edge_count_mult,
			...
			: max_depth(_max_depth), edge_count_mult(_edge_count_mult),
				...
				prune_coeff(_prune_coeff) {}
};

template <typename T>
struct jamana_ehnsw_engine : public ann_engine<T, jamana_ehnsw_engine<T>> {
	...
	float elabel_prob;
	float prune_coeff;
	jamana_ehnsw_engine(jamana_ehnsw_engine_config conf)
	...
	void prune_edges(size_t data_index);
};

template <typename T>
void jamana_ehnsw_engine<T>::_store_vector(const vec<T>& v) {
	all_entries.push_back(v);
}

template <typename T>
void jamana_ehnsw_engine<T>::prune_edges(size_t data_index) {
	for (size_t layer = 0;
			 layer < hadj.size() && hadj[layer].contains(data_index); ++layer) {
		...
		for (const auto& bin : edge_ranks[layer][data_index]) {
			...
			for (const auto& [_, incident_index_offset] : bin) {
				...
				for (size_t chosen_index : chosen_bin_edges) {
					if (prune_coeff * dist(all_entries[chosen_index],
																 all_entries[incident_index]) <=
							dist(all_entries[incident_index], all_entries[data_index])) {
						choose_this = false;
						break;
					}
				}
				if (choose_this) {
					chosen_bin_edges.emplace_back(incident_index);
					final_edges.emplace_back(incident_index);
				}
			}
		}

		hadj[layer][data_index] = final_edges;
	}
}

template <typename T>
void jamana_ehnsw_engine<T>::add_edge_directional(size_t layer, size_t i,
																									size_t j) {
...
template <typename T>
void jamana_ehnsw_engine<T>::add_edge(size_t layer, size_t i, size_t j) {
	add_edge_directional(layer, i, j);
	add_edge_directional(layer, j, i);
}

template <typename T> void jamana_ehnsw_engine<T>::_build() {
...
jamana_ehnsw_engine<T>::_query_k_at_layer(const vec<T>& v, size_t k,
...
template <typename T>
const std::vector<std::vector<size_t>>
jamana_ehnsw_engine<T>::_query_k_internal(const vec<T>& v, size_t k,
...
template <typename T>
std::vector<size_t> jamana_ehnsw_engine<T>::_query_k(const vec<T>& v,
																										 size_t k) {
